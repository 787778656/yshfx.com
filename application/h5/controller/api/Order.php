...
class Order extends Common
{
    ...
    public function confirm()
    {
        ...
        if (empty($this->userInfo['server'])) {
            return json_encode(['code' => 302, 'msg' => '您暂未购买VIP服务']);
        }

        if (count($data) > 1 && $this->userInfo['server'] == 'vip1') {
            return json_encode(['code' => 302, 'msg' => '当前VIP等级无法组合跟单']);
        }
        $time = time();
        $count = 0;
        if(!empty($data)) {
    ...
    public function update_order()
    {
        ...
        if ($user_count && $this->userInfo['server'] == 'vip1') {
            return json_encode(['code' => 302, 'msg' => '当前VIP等级无法组合跟单']);
        }
        $info = Db::name('Mt4DiyAccount')->where($map)->find();
