...
class Trade extends Command
{
    ...
    {
        $this->setName('trade')->setDescription('EA history trade.');
    }
    ...
    protected function execute(Input $input, Output $output)
    {
    	try {
	        // 跟随信号
	        $arrMt4['account'] = Db::name('mt4_account')->where('show', 'in', [1,4])->column('mt4id');

	        // user mt4
	        $arrMt4['user'] = Db::name('user_mt4')->where(['status'=>1, 'mt4server'=>['notlike', '%Demo%']])->column('mt4id');

	        $arrAccount = array_merge($arrMt4['account'], $arrMt4['user']);

	    	if ( ! empty($arrAccount)){
	    		// 清除数据
	    		Db::execute("truncate `zncenter_mt4_account_statistics`;");

		    	foreach ($arrAccount as $key => $value) {
		    		echo '---------'.$value.'------------';
		    		if($value == 810007) continue;
		    		$this->total($value);
		    		$output->writeln("TradeCommand:success...$value");	    		
		    	}
	    	}
    	} catch (\Exception $e) {
        	echo $e->getMessage();
        	//continue;
            //exit('cmd-close: '.$e->getMessage());
        }
    }

    private function total($account = 0)
    {
    	try {
	    	if (!empty($account)){
	        	// 开始更新表
		    	...
		    	if ( ! empty($orders)){	    		
					...
					// 连续盈利/亏损
					list($maxNum, $max) = $this->getMax($result['sprofit']);
					$data['sprofit_max'] = $max;
					$data['sprofit_max_num'] = $maxNum;


					list($maxNum, $max) = $this->getMax($result['sloss']);
					$data['sloss_max'] = $max*(-1);
					$data['sloss_max_num'] = $maxNum;


					list($moreNum, $more) = $this->getMore($result['sprofit']);
					$data['sprofit_more'] = $more;
					$data['sprofit_more_num'] = $moreNum;

					list($moreNum, $more) = $this->getMore($result['sloss']);
					...
					if (!empty($data)){
						if($Account->save($data)){
							...
							$dataAccount['rand_profit'] = implode(',', $this->get_array_value($result['total_profit']));
    ...
    // (总和)最大的连续(盈利/亏损)
    private function getMax($arr = null)
    {
    	if (empty($arr)) return;
    ...
    // (笔数)最多的连续(盈利/亏损)
    private function getMore($arr = null)
    {
    	if (empty($arr)) return;
		foreach ($arr as $key => $value) {
			if (empty($value)) return;
    ...
    // 从数据中取等间距随机值
    private function get_array_value($arr = null, $num = 10){
    	if ( ! empty($arr)){
